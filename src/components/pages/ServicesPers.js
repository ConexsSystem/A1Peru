...
import { TbEye, TbReceipt, TbTrash, TbShare, TbEdit } from "react-icons/tb";
...
const ServicesPersonal = () => {
  ...
  // Función que consulta los servicios y acepta un parámetro para mostrar o no el loader
  ...
  // Efecto inicial: se carga la data con loader y luego se refresca cada 5 segundos sin mostrar el loader
  useEffect(() => {
  ...
  const compartirPorWhatsApp = (url, codigoSeguridad) => {
    const mensaje = encodeURIComponent('¡Hola! Te comparto mi viaje: ' + url + ' \nEl código de seguridad es: ' + codigoSeguridad);
  ...
  return (
    <div className='page'>
      ...
      <Topbar
        ...
        exportFileName="Servicios_personal.xlsx"  // Opcional: nombre del archivo
        exportSheetName="Servicios personal"      // Opcional: nombre de la hoja
      />
              ...
              filteredServices.map((service) => (
                <tr key={service.idreserva}>
                  <td>
                    ...
                    <TbShare
                      className='TbShare'
                      onClick={() => compartirPorWhatsApp(`https://intranet.a1perucorp.com/tracking?key=${service.tokenvalida}`, service.codigoseguridad)}
                    />
                    {
                      service.idestado === 9 ||
                        service.idestado === 19 ||
                  ...
                  <td onClick={() => copyCode(service.codigoseguridad)} style={{ cursor: 'pointer', textAlign: 'center', textDecoration: 'underline' }}>
                    {service.codigoseguridad}
                  </td>
